...
do
	...
	fastp -i "$i"R1.fastq.gz -I "$i"R2.fastq.gz -o $OUTPUT/"$name"_trimmed_fastp_R1.fastq.gz -O $OUTPUT/"$name"_trimmed_fastp_R2.fastq.gz  --length_required="$LENGTH" --qualified_quality_phred="$QUAL" --correction --trim_tail1=1 --trim_tail2=1 --json $OUTPUT/"$name" --html $OUTPUT/"$name"  --report_title="$name" 
